...
| 1.2 | Backend `.env` template (Twilio, Google, DB, Email, AI) | ✅ | `backend/.env.sample` |
...
| 1.5 | Architecture note (stack + deterministic vs AI logic split) | ⬜ | No `/docs` files exist yet |

### PostgreSQL Schema (`sql/001_schema.sql`)
| # | Table | Status | Notes |
|---|-------|--------|-------|
| 1.6 | `businesses` table | ✅ | Includes `config JSONB` field |
| 1.7 | `services` table (multilingual: `name_en`, `name_hi`, `name_zh`) | ✅ | |
| 1.8 | `customers` table | ✅ | |
...
| 1.21 | Test vs production URL strategy documented | ⬜ |

### Twilio Setup
| # | Feature | Status |
|---|---------|--------|
| 1.22 | Twilio voice number configured | ⬜ |
| 1.23 | Twilio webhook pointed at n8n production URL | ⬜ |
| 1.24 | Basic TwiML response verified on a real phone call | ⬜ |
...
| # | State | Status |
|---|-------|--------|
| 3.1 | `LANGUAGE_MENU` (English / Hindi / Mandarin / 9=repeat) | ⬜ |
| 3.2 | `MAIN_MENU_EN` | ⬜ |
| 3.3 | `MAIN_MENU_HI` | ⬜ |
| 3.4 | `MAIN_MENU_ZH` | ⬜ |
...
| 3.15 | `END_CALL` | ⬜ |
| 3.16 | `current_state` stored in `calls` table, updated on every transition | ⬜ |
| 3.17 | Both DTMF digits and `SpeechResult` parsed | ⬜ |
| 3.18 | Key 9 repeats current menu in all states | ⬜ |

### n8n Workflow 1: `twilio-incoming-call-router`
| # | Feature | Status |
|---|---------|--------|
| 3.19 | Webhook receives Twilio POST, normalises payload | ⬜ |
| 3.20 | Lookup call by `CallSid`; create call row if new | ⬜ |
| 3.21 | Load active call context; route by `current_state` | ⬜ |
| 3.22 | Build TwiML for each state (language menu, main menus, service menu, etc.) | ⬜ |
| 3.23 | Parse user selection (digits + speech fallback) | ⬜ |
| 3.24 | Update call state in DB after each valid action | ⬜ |
| 3.25 | Execute sub-workflows for quote+availability, create-booking, cancel-booking | ⬜ |
...
| 3.29 | Return quote + slots to router workflow | ⬜ |
...
|-------|-------|------|-----------|
| Week 1 — Foundation & Schema | 24 | 18 | 6 |
| Week 2 — Quote & Availability | 13 | 0 | 13 |
| Week 3 — IVR, Booking & Cancellation | 27 | 0 | 27 |
| Week 4 — Reminders, Dashboard & Handover | 31 | 0 | 31 |
| **Total** | **95** | **18** | **77** |

### What is done
- Complete PostgreSQL schema with all 9 tables, indexes, and seed data for both business types
- Backend and dashboard `.env` templates
- `.gitignore`
- Project folder structure

### What is next (in order per blueprint)
1. Architecture note in `/docs` (finish Week 1)
2. n8n hello-world webhook + Twilio basic plumbing (finish Week 1)
3. Quote engine + availability engine with Google Calendar (Week 2)
4. Twilio call state machine + booking + cancellation n8n workflows (Week 3)
5. Reminders + provider summary + Next.js dashboard (Week 4)
6. Reschedule flow, customer web pages, AI fallback layer (post-MVP)
